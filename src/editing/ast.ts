...
interface LanguagePatterns {
  /** Patterns to find function/method definitions */
  functionPatterns: RegExp[];
  /** Patterns to find class/struct definitions */
  classPatterns: RegExp[];
  /** Patterns to find interface/trait definitions */
  interfacePatterns: RegExp[];
  /** Patterns to find import statements */
  importPatterns: RegExp[];
  /** Patterns to find type aliases */
  typePatterns: RegExp[];
  /** Special patterns for this language */
  specialPatterns?: RegExp[];
  /** How to find the end of a block (brace-based vs indentation-based) */
 ...
 */
function findMatchingBrace(code: string, openOffset: number): number {
  let depth = 1;
  let inSingle = false;
  let inDouble = false;
 ...
 */
function findIndentBlockEnd(code: string, startLine: number): number {
  const lines = code.split('\n');
  if (startLine >= lines.length) return code.length;
...
export function analyzeStructure(
  ...
  if (!code || language === 'unknown') return [];
  ...
  for (const pattern of patterns.functionPatterns) {
    ...
    let match: RegExpExecArray | null;
    while ((match = pattern.exec(code)) !== null) {
      if (processed.has(match.index)) continue;
  ...
  for (const pattern of patterns.classPatterns) {
    ...
    while ((match = pattern.exec(code)) !== null) {
      ...
      const endPos = offsetToPosition(code, endOffset);
      const structureType: StructureType = language === 'go'
  ...
  if (patterns.interfacePatterns.length > 0) {
    for (const pattern of patterns.interfacePatterns) {
      ...
      while ((match = pattern.exec(code)) !== null) {
        ...
        const endPos = offsetToPosition(code, endOffset);
        const structureType: StructureType = language === 'rust' ? 'trait' : 'interface';
        nodes.push({
          type: structureType,
          name,
          range: { start: startPos, end: endPos },
 ...
 */
function findFunctionsInBlock(code: string, language: SupportedLanguage): StructuralNode[] {
  const nodes: StructuralNode[] = [];
  const patterns = LANGUAGE_PATTERNS[language];
  if (!patterns) return nodes;
  ...
  while ((match = methodPattern.exec(code)) !== null) {
    ...
    if (['if', 'while', 'for', 'switch', 'catch', 'else', 'then'].includes(name)) continue;
 ...
 */
export function validateSyntax(code: string, language: SupportedLanguage): boolean {
  if (language === 'python') {
 ...
 */
function checkBalanced(code: string, open: string, close: string): boolean {
  let depth = 0;
  let inSingle = false;
  let inDouble = false;
  let inTemplate = false;
  ...
  for (let i = 0; i < code.length; i++) {
    ...
    if (!inSingle && !inDouble && !inTemplate && !inBlockComment) {
      if (ch === '/' && next === '/' && !inLineComment) { inLineComment = true; continue; }
      if (ch === '/' && next === '*' && !inBlockComment) { inBlockComment = true; continue; }
    }
    if (ch === '\n') { inLineComment = false; }
 ...
 */
export function formatStructureSummary(nodes: StructuralNode[]): string {
  if (nodes.length === 0) return '(No structural elements found)';
...
export function getStructureIcon(type: StructureType): string {
