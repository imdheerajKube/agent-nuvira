...
export function addParameter(
  ...
  const node = found.node;
  let paramList: ts.Node | null = null;
...
export function changeSignature(
  ...
  if (updateCallSites) {
    ...
    modified = modified.replace(callPattern, (match) => {
      ...
      if (match === nodeText) return match;
 ...
 */
export function detectTransformType(description: string): 'rename' | 'extract' | 'inline' | 'add-param' | 'change-sig' | null {
  const lower = description.toLowerCase();
  if (/rename|change name/i.test(lower)) return 'rename';
  if (/extract|pull out|isolate|separate/i.test(lower)) return 'extract';
  if (/inline|replace call|flatten/i.test(lower)) return 'inline';
  if (/add param|new param|additional param/i.test(lower)) return 'add-param';
  if (/change sign|change signature|modify sign|modify signature|new sign/i.test(lower)) return 'change-sig';
  return null;
}
