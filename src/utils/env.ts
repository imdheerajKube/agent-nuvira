 ...
 */
function parseValue(rawValue: string, env: Record<string, string>): string {
 ...
 */
function resolveVar(varName: string, env: Record<string, string>): string | null {
  if (varName in env) return env[varName];
  if (varName in process.env) return process.env[varName] as string;
  return null;
}
