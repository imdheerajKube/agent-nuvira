...
import type { Skill, SkillStep, SkillParameter, CompilationResult } from './skill-types.js';
import {
  MIN_TRAJECTORIES_FOR_COMPILATION,
  MAX_TRAJECTORIES_FOR_COMPILATION,
  MIN_TRAJECTORY_SCORE_FOR_COMPILATION,
} from './skill-types.js';
...
export class SkillCompiler {
  ...
  async compile(
    ...
    if (candidates.length < MIN_TRAJECTORIES_FOR_COMPILATION) {
      if (verbose) {
        logger.info(
          `   Skill compilation: need ${MIN_TRAJECTORIES_FOR_COMPILATION}+ trajectories ` +
    ...
    const avgScore =
      candidates.reduce((sum, t) => sum + t.score, 0) / candidates.length;
   ...
   */
  static formatSkill(skill: Skill, detailed: boolean = false): string {
    const lines: string[] = [
  ...
  private buildCompilationPrompt(trajectories: Trajectory[]): string {
  ...
  private parseSkills(response: string): Array<{
    name: string;
    description: string;
    version: string;
    goalPattern: string;
  ...
  private finalizeSkill(
    ...
    const now = Date.now();
    const avgScore =
      sourceTrajectories.reduce((sum, t) => sum + t.score, 0) / sourceTrajectories.length;
...
let compilerInstance: SkillCompiler | null = null;

export function getSkillCompiler(): SkillCompiler {
  if (!compilerInstance) {
...
export function resetSkillCompiler(): void {
