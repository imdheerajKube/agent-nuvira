...
function readBenchmarkData(): BenchmarkData {
  try {
    ensureDir();
 ...
 */
export function scoreQuality(output: string, task: BenchmarkTask): number {
  let score = 0.5; // Start at neutral

  // Bonus: output matches expected patterns
  if (task.expectedPatterns) {
    const matched = task.expectedPatterns.filter((p) =>
  ...
  if (task.antiPatterns) {
    const antiMatched = task.antiPatterns.filter((p) =>
      output.toLowerCase().includes(p.toLowerCase()),
    ).length;
 ...
 */
export function formatBenchmarkReport(run: BenchmarkRun): string {
  const lines: string[] = [];
  const s = run.summary;
 ...
 */
export function formatBenchmarkJSON(run: BenchmarkRun): string {
 ...
 */
export function formatBenchmarkMarkdown(run: BenchmarkRun): string {
  const s = run.summary;
  const elapsed = ((run.endedAt - run.startedAt) / 1000).toFixed(1);
 ...
 */
export function getLatestBenchmarkRun(provider: string, model: string): BenchmarkRun | null {
  const data = readBenchmarkData();
 ...
 */
export function compareBenchmarks(runA: BenchmarkRun, runB: BenchmarkRun): string {
  const a = runA.summary;
  const b = runB.summary;

  const winner = (aVal: number, bVal: number, higherBetter: boolean): string => {
