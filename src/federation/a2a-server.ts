...
const taskResults = new Map<string, A2ATaskResult>();
...
function handleAgentCard(_req: IncomingMessage, res: ServerResponse, agentCard: AgentCard): void {
...
async function handleTaskDelegation(
  ...
  try {
    body = (await parseBody(req)) as A2ATaskRequest;
  } catch {
...
async function executeTask(task: A2ATask, _agentCard: AgentCard): Promise<void> {
  task.status = 'running';
...
function handleTaskStatus(req: IncomingMessage, res: ServerResponse): void {
 ...
 */
export function createA2AServer(options: A2AServerOptions = {}): ReturnType<typeof createServer> {
 ...
 */
export function startA2AServer(options: A2AServerOptions = {}): Promise<ReturnType<typeof createServer>> {
  const port = options.port || A2A_DEFAULT_PORT;
  const host = options.host || A2A_DEFAULT_HOST;
