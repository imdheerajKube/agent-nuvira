...
export class TrajectoryStore {
  ...
  async summarize(
  ...
  ): Promise<{ summarized: number; merged: number }> {
    const data = readTrajectories();
    ...
    if (oldTrajectories.length < 3) {
      if (verbose) logger.info(`Only ${oldTrajectories.length} old trajectories — skipping summarization (need 3+)`);
      return { summarized: 0, merged: 0 };
    }

    // Group by project fingerprint for merging
    const groups = new Map<string, typeof oldTrajectories>();
    ...
    for (const [fingerprint, group] of groups) {
      ...
      bestTraj.fileChanges = this.mergeFileChanges(
        bestTraj.fileChanges,
        rest.map(([, t]) => t.fileChanges).flat(),
      );
  ...
  private mergeFileChanges(
    primary: Trajectory['fileChanges'],
    secondary: Trajectory['fileChanges'],
  ...
  getCompressionStats(): {
    ...
    oldTrajectories: number;        // > 30 days
    lowScoreTrajectories: number;    // score < 0.3
    mergeableGroups: number;         // groups of 2+ with same project fingerprint
    estimatedOptimization: string;   // e.g., "~40% reduction possible"
  } {
    const data = readTrajectories();
    ...
    const reductionEstimate = Math.min(
      100,
      Math.round(((oldTrajectories + lowScoreTrajectories * 0.5) / Math.max(1, trajs.length)) * 100),
    );
