 ...
 */
export class DefaultTestModule implements TestModule {
   ...
   */
  private detectFramework(workingDir: string): string {
    if (existsSync(join(workingDir, 'vitest.config.ts')) || existsSync(join(workingDir, 'vitest.config.js'))) return 'vitest';
    if (existsSync(join(workingDir, 'jest.config.js')) || existsSync(join(workingDir, 'jest.config.ts'))) return 'jest';
    if (existsSync(join(workingDir, 'pytest.ini'))) return 'pytest';
    if (existsSync(join(workingDir, 'go.mod'))) return 'go test';
    return 'auto-detect';
  }
   ...
   */
  private applyChanges(sandboxPath: string, changes: FileChange[]): void {
   ...
   */
  private hasDependencies(sandboxPath: string): boolean {
    try {
      const pkg = JSON.parse(readFileSync(join(sandboxPath, 'package.json'), 'utf-8')) as Record<string, unknown>;
      for (const key of ['dependencies', 'devDependencies', 'peerDependencies', 'optionalDependencies']) {
        const section = pkg[key];
        if (section && typeof section === 'object' && Object.keys(section as Record<string, unknown>).length > 0) {
          return true;
        }
      }
      return false;
