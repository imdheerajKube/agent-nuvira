...
type IssueSource = 'github' | 'gitlab' | 'auto';

/** Operation types */
type TriageOperation = 'triage-all' | 'triage-specific' | 'classify' | 'list-unlabeled';
...
export class IssueTriageAgent extends Agent {
  ...
  async execute(context: AgentContext, callLLM: LLMCallFn): Promise<AgentResult> {
    try {
      ...
      this.ghToken = (context.metadata?.githubToken as string) ||
                     (context.metadata?.githubApiKey as string) ||
                     process.env.GH_TOKEN || process.env.GITHUB_TOKEN || '';
      this.glToken = (context.metadata?.gitlabToken as string) ||
                     process.env.GITLAB_TOKEN || process.env.GITLAB_ACCESS_TOKEN || '';

      const taskDesc = context.taskPlan.find(
        (s) => s.agentType === 'issue-triage' && s.status === 'running',
      ...
      const maxIssues = (context.metadata?.maxIssuesToTriage as number) || DEFAULT_MAX_ISSUES;
  ...
  private detectOperation(description: string): TriageOperation {
  ...
  private detectSource(description: string): IssueSource {
    const lower = description.toLowerCase();
    ...
    if (this.glToken && !this.ghToken) return 'gitlab';
    if (this.ghToken && !this.glToken) return 'github';
  ...
  private async resolveGitHubRepo(context: AgentContext, description: string): Promise<RepoInfo | null> {
    ...
    const metaOwner = context.metadata?.githubOwner as string | undefined;
    const metaRepo = context.metadata?.githubRepo as string | undefined;
  ...
  private async resolveGitLabProject(context: AgentContext, description: string): Promise<number | null> {
    ...
    if (typeof metaId === 'number') return metaId;
    if (typeof metaId === 'string' && /^\d+$/.test(metaId)) return Number(metaId);

    const metaPath = context.metadata?.gitlabProjectPath as string | undefined;
    if (metaPath) {
      try { return (await this.gitlabClient.getProject(metaPath)).id; } catch { /* fall through */ }
    }
    ...
    try {
      ...
      if (match) {
        try { return (await this.gitlabClient.getProject(match[1])).id; } catch { /* fall through */ }
      }
    } catch {
      // Fall through
    }
  ...
  private get ghHeaders(): Record<string, string> {
    const h: Record<string, string> = {
      'Accept': 'application/vnd.github.v3+json',
      'Content-Type': 'application/json',
      'User-Agent': 'agent-nuvira/1.0',
    };
    if (this.ghToken) h['Authorization'] = `Bearer ${this.ghToken}`;
    return h;
  }
  ...
  private async githubGet<T>(path: string): Promise<T> {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) throw new Error('GitHub authentication failed. Set GH_TOKEN or GITHUB_TOKEN.');
      if (response.status === 403) throw new Error('GitHub API rate limit exceeded or access denied.');
  ...
  private async githubPatch<T>(path: string, body: unknown): Promise<T> {
    const response = await fetch(`https://api.github.com${path}`, {
      method: 'PATCH',
      headers: this.ghHeaders,
  ...
  private async githubPost<T>(path: string, body: unknown): Promise<T> {
    const response = await fetch(`https://api.github.com${path}`, {
      method: 'POST',
      headers: this.ghHeaders,
      body: JSON.stringify(body),
  ...
  private async fetchGitHubIssues(repo: RepoInfo, maxIssues: number, unlabeledOnly: boolean): Promise<IssueSummary[]> {
    ...
    const raw = await this.githubGet<any[]>(
      `/repos/${repo.owner}/${repo.repo}/issues?${params}`
    );

    // Filter out pull requests (GitHub API returns PRs as issues)
    const issues = raw.filter((i: any) => !i.pull_request);
  ...
  private async fetchGitLabIssues(projectId: number, maxIssues: number, unlabeledOnly: boolean): Promise<IssueSummary[]> {
  ...
  private async ensureGitHubLabels(repo: RepoInfo, labels: string[]): Promise<void> {
  ...
  private async inferAssigneeFromGitBlame(issueBody: string): Promise<string | undefined> {
    try {
  ...
  private buildClassificationPrompt(issue: IssueSummary): string {
    return [
  ...
  private parseClassificationResponse(response: string): Partial<TriageResult> {
  ...
  private validateClassification(c: unknown): c is IssueClassification {
    return typeof c === 'string' && ['bug', 'feature', 'question', 'docs', 'chore'].includes(c);
  }

  private validatePriority(p: unknown): p is IssuePriority {
    return typeof p === 'string' && ['critical', 'high', 'medium', 'low'].includes(p);
  }

  private validateDifficulty(d: unknown): d is IssueDifficulty {
  ...
  private async listUnlabeledIssues(
    source: IssueSource,
    repoInfo: RepoInfo | null,
    gitlabProjectId: number | null,
    maxIssues: number,
  ...
  private async triageSingleIssue(
    source: IssueSource,
    repoInfo: RepoInfo | null,
    gitlabProjectId: number | null,
    issueNumber: number,
    ...
    if (source === 'github' && repoInfo) {
      try {
        ...
        await this.githubPost(
          `/repos/${repoInfo.owner}/${repoInfo.repo}/issues/${issue.number}/labels`,
    ...
    if (source === 'github' && repoInfo) {
      try {
        await this.githubPost(
          `/repos/${repoInfo.owner}/${repoInfo.repo}/issues/${issue.number}/comments`,
          { body: triageComment },
        );
      } catch {
        // Best-effort comment
  ...
  private async triageAllOpenIssues(
    source: IssueSource,
    repoInfo: RepoInfo | null,
    gitlabProjectId: number | null,
    maxIssues: number,
    ...
    const details = [
      ...
      `**Breakdown:**`,
      ...triaged.map((t) =>
        `  • #${t.issueNumber}: ${t.classification} (${PRIORITY_EMOJI[t.priority]} ${t.priority}, ${t.estimatedDifficulty})`
      ),
      ``,
  ...
  private extractIssueNumber(description: string): number | null {
    const patterns = [
      /#(\d+)/,
      /issue\s*#?(\d+)/i,
    ];
  ...
  private buildTriageComment(result: TriageResult): string {
    const lines: string[] = [
      `## 🤖 Agent-Nuvira Triage`,
      ``,
      `| Field | Value |`,
  ...
  private groupByClassification(results: TriageResult[]): Record<string, number> {
  ...
  private groupByPriority(results: TriageResult[]): Record<string, number> {
    const groups: Record<string, number> = {};
    for (const r of results) {
  ...
  private groupByDifficulty(results: TriageResult[]): Record<string, number> {
    const groups: Record<string, number> = {};
    for (const r of results) {
