 ...
 */
export function installHooks(config: HookConfig): boolean {
 ...
 */
export function removeHooks(repoPath: string): boolean {
 ...
 */
export function getHookStatus(repoPath: string): {
  postCheckout: boolean;
  preCommit: boolean;
  fileWatch: boolean;
} {
 ...
 */
export function detectIssueBranch(branchName: string): {
  issueKey: string;
  type: 'feat' | 'fix' | 'chore';
  description: string;
} | null {
  ...
  if (genericMatch) {
    return {
      type: genericMatch[1] as 'feat' | 'fix' | 'chore',
      issueKey: '',
 ...
 */
export function createAndCheckoutBranch(branchName: string, cwd?: string): string {
 ...
 */
export function detectCommitType(changedFiles: string[]): 'feat' | 'fix' | 'refactor' | 'docs' | 'style' | 'test' | 'chore' {
  const allPaths = changedFiles.join(' ').toLowerCase();

  if (allPaths.includes('test') || allPaths.includes('spec') || allPaths.includes('__tests__')) return 'test';
  if (allPaths.includes('doc') || allPaths.includes('readme') || allPaths.includes('.md')) return 'docs';
  if (allPaths.includes('.css') || allPaths.includes('.scss') || allPaths.includes('style')) return 'style';
  if (allPaths.includes('fix') || allPaths.includes('bug') || allPaths.includes('hotfix')) return 'fix';
  if (allPaths.includes('refactor') || allPaths.includes('clean')) return 'refactor';
  if (allPaths.includes('chore') || allPaths.includes('config') || allPaths.includes('deps')) return 'chore';
  return 'feat';
}
