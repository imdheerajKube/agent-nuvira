...
import type { LLMCallFn, FileChange } from '../agent.js';
import { DefaultVerifyModule, type VerifyModule } from '../verify-module.js';

// ─── Types ──────────────────────────────────────────────────────────────────

/** A GitHub pull request (subset of the API response) */
interface GitHubPR {
  number: number;
  title: string;
  body: string | null;
...
/** A file changed in a PR (from the GitHub API) */
...
/** Parsed repo owner and name from git remote */
interface RepoInfo {
  owner: string;
  repo: string;
}
...
/** Operation types */
type PROperation = 'list' | 'review' | 'review-specific' | 'summarize';
...
export class PRReviewAgent extends Agent {
  ...
  async execute(context: AgentContext, callLLM: LLMCallFn): Promise<AgentResult> {
    try {
      ...
      const taskDesc = context.taskPlan.find(
        (s) => s.agentType === 'pr-review' && s.status === 'running',
  ...
  private detectOperation(description: string): PROperation {
    const lower = description.toLowerCase();

    if (lower.includes('list') || lower.includes('show open') || lower.includes('display')) return 'list';
    ...
    if (lower.includes('summarize') || lower.includes('summary') || lower.includes('describe')) return 'summarize';
    if (lower.includes('review') || lower.includes('all') || lower.includes('open')) return 'review';
    return 'review'; // Default: review all open PRs
  }

  // ─── Repo Resolution ──────────────────────────────────────────────────────

  private async resolveRepoInfo(context: AgentContext): Promise<RepoInfo | null> {
  ...
  private resolveToken(context: AgentContext): string {
    return (context.metadata?.githubApiKey as string) ||
           this.token ||
           '';
  }
  ...
  private async listOpenPRs(repo: RepoInfo): Promise<AgentResult> {
    const prs = await this.githubGet<GitHubPR[]>(
      `/repos/${repo.owner}/${repo.repo}/pulls?state=open&per_page=20`
    );
  ...
  private async reviewSinglePR(repo: RepoInfo, prNumber: number, callLLM: LLMCallFn): Promise<AgentResult> {
  ...
  /** Review all open PRs */
  private async reviewAllOpenPRs(repo: RepoInfo, callLLM: LLMCallFn, context: AgentContext): Promise<AgentResult> {
    const prs = await this.githubGet<GitHubPR[]>(
      `/repos/${repo.owner}/${repo.repo}/pulls?state=open&per_page=10`
    );
    ...
    const maxPRs = (context.metadata?.maxPRsToReview as number) || 5;
    ...
    const details = results.map((r) =>
      `  #${r.prNumber}: ${r.passed ? '✅' : '❌'} ${r.summary.slice(0, 100)} (${r.commentsPosted} comments)`
    ).join('\n');
  ...
  private async summarizePR(repo: RepoInfo, prNumber: number, callLLM: LLMCallFn): Promise<AgentResult> {
    const pr = await this.githubGet<GitHubPR>(`/repos/${repo.owner}/${repo.repo}/pulls/${prNumber}`);
    const files = await this.githubGet<PRChangedFile[]>(
   ...
   */
  private async reviewPR(repo: RepoInfo, prNumber: number, callLLM: LLMCallFn): Promise<PRReviewResult> {
    // Fetch PR details
    const pr = await this.githubGet<GitHubPR>(`/repos/${repo.owner}/${repo.repo}/pulls/${prNumber}`);
  ...
  private extractPRNumber(description: string): number | null {
    const patterns = [
      /#(\d+)/,
      /PR\s*#?(\d+)/i,
   ...
   */
  private extractLineFromPatch(patch: string, issueText: string): number | null {
