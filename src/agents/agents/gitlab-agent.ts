...
import type { GitLabProject, CreateMROptions, CreateIssueOptions } from './gitlab-api-client.js';

// ─── Types ──────────────────────────────────────────────────────────────────

/** Operation types the GitLab agent can perform */
type GitLabOperation = 'mr-create' | 'mr-comment' | 'mr-merge' | 'mr-list' | 'issue-create' | 'issue-comment' | 'issue-list' | 'pipeline-status' | 'discover';
...
export class GitLabAgent extends Agent {
  ...
  async execute(context: AgentContext, _callLLM: LLMCallFn): Promise<AgentResult> {
    try {
      ...
      const taskDesc = context.taskPlan.find(
        (s) => s.agentType === 'gitlab' && s.status === 'running',
  ...
  private detectOperation(description: string): GitLabOperation {
    const lower = description.toLowerCase();

    // Discover projects
    if (lower.includes('discover') || lower.includes('list project') || lower.includes('find project')) return 'discover';
    ...
    if ((lower.includes('create') || lower.includes('new')) && (hasMr || hasMergeRequest)) return 'mr-create';

    // 'list'/'show'/'open' + 'mr'/'merge request'
    if ((lower.includes('list') || lower.includes('show') || lower.includes('open')) && (hasMr || hasMergeRequest)) return 'mr-list';
    ...
    if ((mergeAsVerb || lower.includes('accept') || lower.includes('approve')) && (hasMr || hasMergeRequest)) return 'mr-merge';

    // 'comment'/'review' + 'mr'/'merge request'
    if ((lower.includes('comment') || lower.includes('review') || lower.includes('note')) && (hasMr || hasMergeRequest)) return 'mr-comment';

    // Catch-all for other mr-related descriptions
    if (hasMr || hasMergeRequest) return 'mr-create';

    // Issue operations
    if (lower.includes('create issue') || lower.includes('new issue')) return 'issue-create';
    if (lower.includes('comment') && lower.includes('issue')) return 'issue-comment';
    if (lower.includes('list issue') || lower.includes('open issue') || lower.includes('show issue')) return 'issue-list';

    // Pipeline
    if (lower.includes('pipeline') || lower.includes('ci') || lower.includes('build status')) return 'pipeline-status';
   ...
   */
  private async resolveProjectId(context: AgentContext): Promise<number | null> {
  ...
  private async discoverProjects(): Promise<AgentResult> {
    ...
    const details = projects.map((p) =>
      `  • ${p.nameWithNamespace} (id: ${p.id}) — ${p.webUrl}`
    ).join('\n');
  ...
  private async createMergeRequest(
    ...
    const sourceBranch = this.detectBranch(description) || context.metadata?.gitlabSourceBranch as string || '';
    const targetBranch = this.detectTargetBranch(description) || context.metadata?.gitlabTargetBranch as string || 'main';
  ...
  /** List merge requests */
  private async listMergeRequests(projectId: number | null, description: string): Promise<AgentResult> {
    const state = description.includes('merged') ? 'merged' as const
      : description.includes('closed') ? 'closed' as const
      : 'opened' as const;
  ...
  private async mergeMR(projectId: number | null, description: string): Promise<AgentResult> {
    const mrIid = this.extractMRNumber(description);
  ...
  /** List issues */
  private async listIssues(projectId: number | null, description: string): Promise<AgentResult> {
    const state = description.includes('closed') ? 'closed' as const : 'opened' as const;
    ...
    const details = issues.map((issue) =>
      `  • #${issue.iid} ${issue.title} (${issue.labels.length > 0 ? issue.labels.join(', ') : 'no labels'})`
    ).join('\n');
  ...
  /** Check pipeline status */
  private async checkPipelineStatus(projectId: number | null, _description: string): Promise<AgentResult> {
    if (!projectId) {
      return { success: false, summary: 'No GitLab project resolved', error: 'Set git remote or gitlabProjectId' };
    }
    ...
    const details = pipelines.map((p) =>
      `  • Pipeline #${p.id} — ${p.status} (${p.ref})`
    ).join('\n');
  ...
  private detectBranch(description: string): string | undefined {
  ...
  private detectTargetBranch(description: string): string {
  ...
  private generateMRTitle(description: string, goal: string): string {
  ...
  private generateMRDescription(description: string, goal: string): string {
    const lines: string[] = [];
    lines.push('## Summary');
    lines.push('');
    lines.push(goal);
  ...
  private extractMRNumber(description: string): number | null {
    const patterns = [
      /!(\d+)/,
      /MR\s*#?(\d+)/i,
      /merge request\s*#?(\d+)/i,
  ...
  private extractCommentText(description: string): string | undefined {
