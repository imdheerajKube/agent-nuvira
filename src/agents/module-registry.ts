...
export class ModuleRegistry {
  ...
  register(
    ...
    meta: Omit<ModuleMetadata, 'agentType' | 'isBuiltin'> & { isBuiltin?: boolean },
  ): void {
    if (this.factories.has(agentType)) {
      throw new Error(
        `Agent type '${agentType}' is already registered. Use unregister() first to replace.`,
      );
    }
 ...
 */
export function setModuleRegistry(registry: ModuleRegistry): ModuleRegistry | null {
