...
import type { ReportModule, ExecutionReport, ReportFormat } from './report-module.js';
...
export class TaskExecutionPipeline {
  ...
  private inspectModule: InspectModule;
   ...
   */
  async execute(goal: string, config: PipelineConfig): Promise<PipelineResult> {
    ...
    let editResult: StepResult<EditOutput> = skipStep<EditOutput>();
    let testResult: StepResult<TestOutput> = skipStep<TestOutput>();
    let verifyResult: StepResult<VerificationResult> = skipStep<VerificationResult>();
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      // ── Step 5: Verify ────────────────────────────────────────────
      verifyResult = await this.stepVerify(
        editResult.data!,
        testResult.data,
        planOutput.goal,
        callLLM,
    ...
    // ── Step 6: Summarize ─────────────────────────────────────────────
    const summarizeResult = await this.stepSummarize(
      goal,
      { plan: planResult, inspect: inspectResult, edit: editResult!, test: testResult!, verify: verifyResult! },
      startTime,
      audit,
      verbose,
    );
  ...
  private async stepPlan(
    ...
    audit: PipelineAudit,
    verbose: boolean,
  ): Promise<StepResult<PlanOutput>> {
    const stepStart = Date.now();
    audit.log('plan', 'plan:started', 'info', 'Decomposing goal into tasks');
    if (verbose) logger.highlight('\n📋 Phase 1: Plan');
    if (verbose) logger.info('   Decomposing goal into structured task plan...');
    ...
    try {
      ...
      const output: PlanOutput = { goal, tasks, complexity };
      ...
      this.eventBus.emit('plan:step-created' as any, {
        goal,
        taskCount: tasks.length,
        complexity,
      }, 'task-execution-pipeline');
      ...
      if (verbose) logger.error(`   ❌ Planning failed: ${msg}`);
   ...
   */
  private async stepInspect(
    plan: PlanOutput,
    callLLM: LLMCallFn,
    workingDirectory: string,
    audit: PipelineAudit,
    verbose: boolean,
  ): Promise<StepResult<InspectionResult>> {
    const stepStart = Date.now();
    audit.log('inspect', 'inspect:started', 'info', 'Scanning codebase for relevant files');
    if (verbose) logger.highlight('\n📁 Phase 2: Inspect');
    if (verbose) logger.info('   Scanning codebase for relevant files...');
    ...
    try {
      ...
      audit.log('inspect', 'inspect:completed', 'success',
        `Found ${result.artifacts.length} relevant files (${result.stats.totalFiles} total scanned)`,
        {
      ...
      if (verbose) logger.error(`   ❌ Inspection failed: ${msg}`);
  ...
  private async stepEdit(
    ...
    verbose: boolean,
    isRetry: boolean,
  ): Promise<StepResult<EditOutput>> {
    const stepStart = Date.now();
    const label = isRetry ? ' (retry)' : '';
    audit.log('edit', 'edit:started', 'info', `Generating code changes${label}`);
    if (verbose) logger.highlight(`\n✍️  Phase 3: Edit${label}`);
    if (verbose) logger.info('   Generating code changes...');

    try {
      const changes: EditOutput['changes'] = [];
      ...
      if (changes.length === 0) {
        ...
        if (verbose) logger.warn('   ⚠️  No changes generated');
      } else {
      ...
      const output: EditOutput = { changes, warnings };
      ...
      if (verbose) logger.error(`   ❌ Edit failed: ${msg}`);
   ...
   */
  private async stepTest(
    editOutput: EditOutput,
    callLLM: LLMCallFn,
    workingDirectory: string,
    audit: PipelineAudit,
    verbose: boolean,
  ): Promise<StepResult<TestOutput>> {
    const stepStart = Date.now();
    ...
    if (editOutput.changes.length === 0) {
      ...
      if (verbose) logger.info('   No changes to test — skipping');
      return {
        success: true,
    ...
    try {
      ...
      audit.log('test', 'test:completed', output.passed ? 'success' : 'warning',
        `${output.passedCount}/${output.totalCount} tests passed (exit code: ${exitCode})`,
        { passed: output.passedCount, failed: output.failedCount, total: output.totalCount, exitCode },
      );
      ...
      if (verbose) logger.error(`   ❌ Tests failed: ${msg}`);
      return {
        success: false,
  ...
  private async stepVerify(
    ...
    if (verbose) logger.highlight('\n🔍 Phase 5: Verify');
    if (verbose) logger.info('   Running verification checks (security, alignment, tests)...');
    ...
    try {
      ...
      audit.log('verify', 'verify:completed', result.passed ? 'success' : 'warning',
        `Verification: ${(result.overallScore * 100).toFixed(0)}% score, ${result.blockers.length} blocker(s)`,
        { score: result.overallScore, checks: result.checks.length, blockers: result.blockers.length },
      );
      ...
      if (verbose) logger.error(`   ❌ Verification failed: ${msg}`);
  ...
  private async stepSummarize(
    ...
    audit: PipelineAudit,
    verbose: boolean,
  ): Promise<StepResult<ExecutionReport>> {
    const stepStart = Date.now();
...
5. Write unit tests for auth flow`;
   ...
   */
  private buildEditPrompt(plan: PlanOutput, inspection: InspectionResult, isRetry: boolean): string {
...
- Add appropriate error handling${retryNote}`;
   ...
   */
  private parsePlanResponse(response: string): string[] {
  ...
  private parseEditResponse(
    response: string,
    workingDirectory: string,
  ): { changes: EditOutput['changes']; warnings: string[] } {
    const changes: EditOutput['changes'] = [];
    const warnings: string[] = [];
    ...
    while ((match = blockRegex.exec(response)) !== null) {
      ...
      const status = exists ? 'modified' as const : 'created' as const;
   ...
   */
  private parseTestOutput(output: string, exitCode: number): { passedCount: number; failedCount: number; totalCount: number } {
    ...
    for (const pattern of patterns) {
      ...
      if (match) {
        ...
        if (total > 0) return { passedCount: passed, failedCount: failed, totalCount: total };
      }
    }
   ...
   */
  private detectTestCommand(workingDirectory: string): string {
    ...
    if (existsSync(join(workingDirectory, 'vitest.config.ts'))) return 'npx vitest run';
    if (existsSync(join(workingDirectory, 'jest.config.js'))) return 'npx jest';
    if (existsSync(join(workingDirectory, 'pytest.ini'))) return 'python -m pytest';
    if (existsSync(join(workingDirectory, 'go.mod'))) return 'go test ./...';
  ...
  private applyChanges(
    ...
    for (const change of changes) {
      ...
      try {
        ...
        if (verbose) logger.error(`      ❌ Failed to write ${change.path}: ${msg}`);
      }
    }
  ...
  private emitEvent(event: string, data: Record<string, unknown>): void {
    try {
      (this.eventBus.emit as (event: string, data: unknown, source?: string) => void)(event, data, 'task-execution-pipeline');
    } catch {
 ...
 */
function skipStep<T>(): StepResult<T> {
  return { success: false, summary: 'Skipped', durationMs: 0 };
}

function skipRemaining() {
  return {
    edit: skipStep<EditOutput>(),
    test: skipStep<TestOutput>(),
    verify: skipStep<VerificationResult>(),
    summarize: skipStep<ExecutionReport>(),
  };
}
