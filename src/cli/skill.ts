...
export class SkillCommand {
  ...
  private listSkills(opts: { quality?: number }): void {
  ...
  private showSkill(nameOrId: string): void {
    const store = getSkillStore();
  ...
  private async runSkill(
    ...
    const missing = skill.parameters.filter(
  ...
  private async compileSkills(opts: { provider?: string; model?: string }): Promise<void> {
  ...
  private searchSkills(query: string): void {
    const store = getSkillStore();
  ...
  private showQuality(opts: { details?: boolean }): void {
    const store = getSkillStore();
    const summary = store.getSummary();
