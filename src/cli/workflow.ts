...
export class WorkflowCommand extends BaseCommand {
  create(): Command {
    ...
    command
      ...
      .action(async (query: string, options?: { refresh?: boolean }) => {
  ...
  private async searchRegistry(query: string, options?: { refresh?: boolean }): Promise<void> {
    if (options?.refresh) {
      clearRegistryCache();
    }
  ...
  private async installTemplate(templateId: string): Promise<void> {
  ...
  private async preparePublish(templateId: string): Promise<void> {
    console.log('');
  ...
  private async showInfo(templateId: string): Promise<void> {
