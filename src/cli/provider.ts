...
const PROVIDER_DISPLAY: Record<string, { icon: string; label: string; color: (s: string) => string }> = {};
...
for (const [id, hex] of Object.entries(BRAND_COLORS)) {
  if (PROVIDER_DISPLAY[id]) PROVIDER_DISPLAY[id].color = (s) => chalk.hex(hex)(s);
}
...
const BUILTIN_PROVIDERS: ProviderType[] = [...CATALOG_PROVIDER_IDS];

// ─── Table Helpers ──────────────────────────────────────────────────────────

function statusIcon(available: boolean, configured: boolean): string {
  if (available) return chalk.green('✅');
  if (configured) return chalk.yellow('⚠️');
  return chalk.red('❌');
}

function statusLabel(available: boolean, configured: boolean): string {
  if (available) return chalk.green('Available');
  if (configured) return chalk.yellow('Unreachable');
  return chalk.red('Not configured');
}

function modelLabel(model: string | undefined): string {
  if (!model) return chalk.dim('—');
...
function keyPreview(key: string | undefined): string {
  if (!key) return chalk.dim('Not set');
  return chalk.gray(`${key.slice(0, 6)}...${key.slice(-4)}`);
}

function divider(char = '─', count = 58): string {
...
export class ProviderCommand extends BaseCommand {
  create(): Command {
    ...
    command
      ...
      .action(async (providerName?: string, options?: { watch?: boolean; verbose?: boolean }) => {
        if (options?.watch) {
  ...
  private async listProviders(options: { all?: boolean }): Promise<void> {
    const registry = getPluginRegistry();
    const pluginTypes = registry.getAllPlugins().map((p) => p.getProviderType());
    const providerTypes: string[] = [...BUILTIN_PROVIDERS, ...pluginTypes];

    // Fetch status for all providers in parallel
    const results = await Promise.all(
      providerTypes.map(async (pt) => {
        const isLocal = pt === 'local';
        const hasKey = isLocal || this.configManager.hasRequiredCredentials(pt as ProviderType);
        let available = false;
        let providerName = pt;
    ...
    for (const r of results) {
      const display = PROVIDER_DISPLAY[r.pt] || { icon: '🔌', label: r.pt, color: (s: string) => chalk.hex('#888')(s) };
      ...
      if (r.configured) configuredCount++;
    }
  ...
  private async checkHealth(
    ...
    for (const pt of providersToCheck) {
      const display = PROVIDER_DISPLAY[pt] || { icon: '🔌', label: pt, color: (s: string) => chalk.hex('#888')(s) };
  ...
  private getEndpointDetail(providerType: string): string {
