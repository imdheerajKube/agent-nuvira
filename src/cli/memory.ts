...
export class MemoryCommand extends BaseCommand {
  create(): Command {
    ...
    const optimizeCmd = new Command('optimize')
      ...
      .action(async (options?: { dryRun?: boolean; aggressive?: boolean }) => {
    ...
    const summarizeCmd = new Command('summarize')
      ...
      .action(async (options?: { retention?: number; verbose?: boolean }) => {
  ...
  private async optimize(options: {
    dryRun?: boolean;
    aggressive?: boolean;
  }): Promise<void> {
    const trajStore = getTrajectoryStore();
