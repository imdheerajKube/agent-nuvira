...
export class SDKCommand extends BaseCommand {
  create(): Command {
    ...
    const scaffoldCmd = new Command('scaffold')
      ...
      .action(async (outDir: string, agentName: string, description: string, options: { template?: string; agentType?: string }) => {
    ...
    const registerCmd = new Command('register')
      ...
      .action(async (className: string, agentType: string, sourceModule: string, options: { icon?: string; orchestratorPath?: string }) => {
    ...
    const unregisterCmd = new Command('unregister')
      ...
      .option('--orchestrator-path <path>', 'Path to orchestrator.ts (auto-detected if not set)')
      .action(async (agentType: string, options: { orchestratorPath?: string }) => {
  ...
  private async handleScaffold(
    ...
    agentName: string,
    description: string,
    options: { template?: string; agentType?: string },
  ): Promise<void> {
  ...
  private async handleRegister(
    ...
    agentType: string,
    sourceModule: string,
    options: { icon?: string; orchestratorPath?: string },
  ): Promise<void> {
  ...
  private async handleUnregister(
    agentType: string,
    options: { orchestratorPath?: string },
  ): Promise<void> {
