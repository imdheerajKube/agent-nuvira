  ...
  private async runEval(options: {
    ...
    if (isPlaceholderApiKey(resolvedConfig.apiKey)) {
      ...
      if (options.model) {
        ...
        logger.warn(
          `  ⚠️ ${providerName} is pinned with a placeholder API key ("${resolvedConfig.apiKey}") — every eval task will fail auth. An explicit --model cannot fix a dead key.`,
        );
        logger.warn(tip);
      } else {
        logger.error(
          `  🚫 ${providerName} is pinned with a placeholder API key ("${resolvedConfig.apiKey}") — this is NOT a real key, so every eval task would fail auth.`,
        );
        logger.error(tip);
        logger.error(
