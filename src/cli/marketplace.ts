...
export class MarketplaceCommand extends BaseCommand {
  create(): Command {
    ...
    command
      ...
      .action(async (options?: { workflows?: boolean; plugins?: boolean; refresh?: boolean }) => {
  ...
  private async browse(options: { workflows?: boolean; plugins?: boolean; refresh?: boolean }): Promise<void> {
  ...
  private async search(query: string): Promise<void> {
    ...
    let registryResults: Array<{ id: string; name: string; description: string; author: string; version: string; tags: string[] }> = [];
    ...
    const builtin = getWorkflowTemplates().filter(
      (t) => t.id.includes(query) || t.description.toLowerCase().includes(query.toLowerCase()),
    );

    // Search plugins
    const registry = getPluginRegistry();
    const plugins = registry.getAllPlugins().filter(
      (p) => p.metadata.name.toLowerCase().includes(query.toLowerCase()) ||
  ...
  private async showInfo(name: string): Promise<void> {
    ...
    // Check plugins
    const registry = getPluginRegistry();
    const plugin = registry.getAllPlugins().find(
      (p) => p.metadata.name === name || p.getProviderType() === name,
    );
    if (plugin) {
      ...
      if (plugin.metadata.author) console.log(`  Author: ${plugin.metadata.author}`);
      if (plugin.metadata.version) console.log(`  Version: ${plugin.metadata.version}`);
