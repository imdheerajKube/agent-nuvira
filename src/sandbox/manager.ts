...
import { detectProjectImage } from './images.js';
...
export class SandboxManager {
  ...
  constructor(private config: SandboxConfig = getSandboxConfig()) {}
  ...
  async createContainer(
    ...
    if (!(await this.isDockerAvailable())) {
      throw new Error(
        'Docker is not available. ' + (this.dockerError || 'Is Docker installed and running?'),
      );
    }
    ...
    if (this.containers.size >= MAX_CONTAINERS) {
      ...
      if (this.containers.size >= MAX_CONTAINERS) {
        throw new Error(
          `Maximum of ${MAX_CONTAINERS} sandbox containers reached. ` +
    ...
    if (runResult.exitCode !== 0) {
      throw new Error(
        `Failed to create sandbox container: ${runResult.stderr || runResult.stdout}`,
      );
    }
  ...
  async copyProjectToContainer(
    ...
    const visibleEntries = entries.filter((e: string) => !e.startsWith('.') && !excluded.has(e));
    ...
    const tarArgs = visibleEntries.map((e: string) => `"${join(sourceDir, e)}"`).join(' ');
    ...
    if (result.exitCode !== 0) {
      ...
      for (const entry of visibleEntries) {
        try {
          await this.execHostCommand(
            `docker cp "${join(sourceDir, entry)}" "${containerId}:${resolvedTarget}/"`,
            60_000,
          );
        } catch {
  ...
  async getContainerStatus(containerId: string): Promise<ContainerStatus> {
    ...
    try {
      const result = await this.execHostCommand(
  ...
  async destroyContainer(containerId: string): Promise<void> {
    ...
    try {
      ...
      await this.execHostCommand(
        `docker stop --time ${Math.floor(this.config.stopTimeoutMs / 1000)} "${containerId}" 2>/dev/null`,
        30_000,
      );
    } catch {
      // Ignore stop errors
    ...
    try {
      ...
      await this.execHostCommand(
        `docker rm -f "${containerId}" 2>/dev/null`,
        30_000,
      );
    } catch {
   ...
   */
  private ensureContainer(containerId: string): ManagedContainer {
  ...
  private async cleanupStaleContainers(): Promise<void> {
    ...
    const stale = [...this.containers.entries()].filter(
      ([_, c]) => c.createdAt < staleTime,
    );
