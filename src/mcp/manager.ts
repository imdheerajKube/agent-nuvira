...
export class MCPManager {
  ...
  async connect(name: string): Promise<MCPClient> {
    ...
    if (config.transport !== 'sse') installProcessCleanup();
   ...
   */
  async connectAll(): Promise<MCPClient[]> {
    const configs = this.discoverConfigs();
  ...
  getAllTools(): Array<{ server: string; tool: Tool }> {
    const all: Array<{ server: string; tool: Tool }> = [];
   ...
   */
  async callTool(name: string, args?: Record<string, unknown>): Promise<{ server: string; result: CallToolResult } | null> {
