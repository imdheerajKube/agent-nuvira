...
| **v1.60.0** | Aug 2026 | **No hardcoded model defaults — fully dynamic selection** — `defaultProvider` is now `'auto'`, resolved at runtime to the best *available* provider for your keys + learned registry (verified → configured-with-key → zero-config local); every provider's model pin is the `'default'` sentinel resolved to a registry-verified working model at call time (explicit pins still win); `src/learning/model-selection.ts` is the single selection authority; fallback chains built from what you configured — never a fixed list; 3,395 tests |
| **v1.60.1** | Aug 2026 | **Live per-model context windows feed the router's context preflight** — the probe records each provider-advertised window into the registry (Ollama `/api/tags`, OpenRouter `/models`); `resolveContextWindow` precedence: user override → live registry descriptor → provider estimate; `ModelDescriptor`/`ModelRegistryEntry` gain `contextWindowTokens`; 3,398 tests |
| **v1.60.2** | Aug 2026 | **Live context windows for every provider that exposes one** — Ollama multi-source parsing (`details.context_length` 0.32.x, `general`/`llama`/family-keyed `model_info`) + bounded `/api/show` fallback (max 8 — `gemma4:e4b` → 131,072); Gemini `inputTokenLimit` (gated on `generateContent`); NIM `max_model_len` (vLLM); Groq documented as not exposing one; 3,404 tests |
| **v1.60.3** | Aug 2026 | **Reasoning traces + failure-lesson memory + goal-fidelity planning** — P0 per-step reasoning-trace capture (`buff trace list/show/replay/clear`, dashboard 🔍 panel: agent × model × prompt digest × tokens × latency × routing snapshot); P1 self-improver distills negative trajectories into episodic failure-lesson memory injected into future planning prompts; **planner goal-fidelity guard** rejects off-topic / few-shot-regurgitated plans (the NVDA-addon failure mode) with an actionable error; **planner-repair model escalation** re-routes a failed planner through the Auto router at the next complexity level; 3,447 tests |
...
| **v1.61.0** | Aug 2026 | **Routing + config-hygiene program (Issues 001–004)** — dynamic 17+ provider catalog joins routing (no hardcoded defaults); the router leverages gathered data (degraded-provider pre-filter, bandit ON, transparent context-window sources, cited exclusion reasons); the full router feature set fires at every action point (plan/eval/benchmark/model explain/edit --auto-route); invalid API keys auto-cleared after 3 consecutive 401/403s with clear errors + stale local models pruned on `buff models refresh` (verified entries demoted, telemetry preserved); 3,510 tests |
| **v1.59.9** | Aug 2026 | **Config-path consistency** — `BUFF_CONFIG_DIR` honored by the config manager, dashboard readers, and vector-store backend picker via one shared resolver (explicit dir > `$BUFF_CONFIG_DIR` > `~/.buff`); hermetic runs can never leak into the real `~/.buff/buffconfig.json`; 3,367 root tests |
...
| **v1.59.4** | Aug 2026 | P6 M6.6 Software Bill of Materials: `buff sbom` (CycloneDX 1.5, reproducible) + `sbom verify` (drift/tamper) + `sbom licenses` + doctor Supply Chain check; 3,313 tests |
| **v1.59.3** | Aug 2026 | `config set routing.<gate>` accepts the boolean soft-signal keys (`capabilityFit`, `contextFit`, `partialFlakiness`); 3,286 tests |
| **v1.59.2** | Aug 2026 | P4 M4.4 partial flakiness feeds the ROUTER: registry `partialRate` EMA (mid-stream interruptions bump, clean successes heal, never flips status) → reliability penalty gated by `routing.partialFlakiness` (default ON) + `⏸ flaky` chip in `models explain`; 3,284 tests |
...
| **v1.59.0** | Aug 2026 | P6 Enterprise Hardening begins: M6.2 secret-redaction scrubber (logger + audit writers) + M6.3 tamper-evident hash-chained audit (`buff audit verify/export`, tamper detection in `doctor --enterprise`); 3,275 tests |
