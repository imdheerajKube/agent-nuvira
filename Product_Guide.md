...
| 103 | **Context-Length Preflight (Nuvira-Router P2-M2.5)** | ✅ Complete (M2.5) | next release | The router scores each provider's nominal input context window against the task's estimated prompt size before picking — the genuine gap Copilot identified. Built-in per-model/per-provider window tables with `routing.contextWindows` overrides (model or provider keys); the estimate uses the caller's `contextHintTokens` when the real payload is known (chat passes the growing conversation + full prompt), else the task text. SOFT, estimation-only, NEVER a hard block: neutral below 50% utilization (normal tasks never shift a ranking), ramp to a capped 35% penalty; models may exceed nominal windows. Reversible `routing.contextFit` gate (default ON). `models explain` shows a ⏳ ctx chip per ranked row + a Context-preflight section + JSON `context` (basis task|hint, per-provider window/utilization/fit); fallback chains carry `contextWindowTokens`; `buff config set routing.contextWindows.<model|provider> <tokens>`. A 500K-token payload flips privacy-first local → gemini (1M window) |
