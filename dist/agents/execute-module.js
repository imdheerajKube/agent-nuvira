 ...
 */
export class DefaultExecuteModule {
     ...
     */
    inferCommand(goal, fileChanges, _artifactPaths) {
        ...
        if (lower.includes('npm test') || lower.includes('run test'))
            return 'npm test';
        if (lower.includes('npm start') || lower.includes('run start'))
            return 'npm start';
        if (lower.includes('npm run build') || lower.includes('run build'))
            return 'npm run build';
        if (lower.includes('python')) {
            const pyMatch = goal.match(/python\s+([a-zA-Z0-9_./\\-]+\.[a-zA-Z0-9]+)/i);
            if (pyMatch)
                return `python ${pyMatch[1]}`;
        }
        ...
        if (fileChanges) {
            for (const fc of fileChanges) {
                if (fc.path.endsWith('.py'))
                    return `python ${fc.path}`;
                if (fc.path.endsWith('.js') || fc.path.endsWith('.mjs'))
                    return `node ${fc.path}`;
                if (fc.path.endsWith('.sh'))
                    return `bash ${fc.path}`;
                if (fc.path.endsWith('.go'))
                    return `go run ${fc.path}`;
                if (fc.path.endsWith('.rs'))
                    return `cargo run`;
            }
        }
        return null;
