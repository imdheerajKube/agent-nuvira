...
export class MCPAgent extends Agent {
    ...
    async execute(context, _callLLM) {
        try {
            ...
            if (!request) {
                ...
                const tools = context.metadata.mcpTools;
     ...
     */
    parseToolCallFromDescription(description) {
        // Pattern 1: callTool('name', { ... })
        const callToolMatch = description.match(/callTool\s*\(\s*['"]([^'"]+)['"]\s*(?:,\s*(\{[\s\S]*?\}))?\s*\)/);
        if (callToolMatch) {
...
export function formatMcpToolsForPrompt(tools) {
    ...
    parts.push('Tool call results are available to subsequent steps via the context.', 'The PlannerAgent can schedule `mcp` steps to invoke tools directly.');
