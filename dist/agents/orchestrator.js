...
export class Orchestrator {
    ...
    async executePipeline(goal, options = {}) {
        ...
        for (let iteration = 0; iteration < 50; iteration++) {
            ...
            if (parallelGroup.length > 0) {
                ...
                await Promise.all(parallelGroup.map(({ task, strategy }) => this.executeSingleTask(task, vault, options, agentResults, contextFiles, defaultCallLLM, strategy)));
            }
        ...
        this.stats.rollbackCount = vault.context.fileChanges.filter((c) => c.status === 'modified' && c.newContent !== undefined && c.newContent === c.originalContent).length;
     ...
     */
    runProjectInspection(vault, options) {
        ...
        try {
            ...
            try {
                ...
                const extra = [];
                if (pkg.name)
                    extra.push(`name: ${pkg.name}`);
                if (pkg.scripts?.test)
                    extra.push('test script present');
                if (pkg.scripts?.build)
                    extra.push('build script present');
                if (extra.length > 0)
                    lines.push(`package.json — ${extra.join(' · ')}`);
            }
            catch {
            ...
            lines.push(`${sourceCount} source file(s)` +
    ...
    countSourceFiles(cwd) {
        ...
        const walk = (dir, depth, inTestDir) => {
            if (depth > 6)
                return;
            let entries;
            try {
        ...
        return async (info) => {
            ...
            if (action === 'switch-model') {
                ...
                if (picked.provider === 'auto' || isAutoModel(picked.model)) {
                    ...
                    newCallLLM = this.createAutoRoutedLLM({ agentType: info.agentName || 'chat', description: 'Rate-limit retry' }, options);
                }
                else {
                        ...
                        if (chunks > 0) {
                            ...
                            try {
                                const originalTokens = contextFiles.reduce((sum, f) => {
                                    try {
                                        return sum + retrievalEstimateTokens(readFileSync(f, 'utf-8'));
                                    }
                                    catch {
                                        return sum;
                                    }
    ...
    getExecutionStrategy(task, routingContext) {
        ...
        switch (intent) {
            case 'security':
                if (writerLike)
                    effectiveAgentType = 'security';
                runSerially = true;
                useRepair = true;
                maxRepairs = 4;
                break;
            case 'debugging':
                if (writerLike || task.agentType === 'runner')
                    effectiveAgentType = 'debugger';
                runSerially = true;
                useRepair = true;
                maxRepairs = 5;
                break;
