...
export function discoverWorkflowPlugins() {
    ...
    ensureDirectories();
    try {
        const files = readdirSync(WORKFLOWS_DIR).filter((f) => f.endsWith('.json') || f.endsWith('.yaml') || f.endsWith('.yml'));
        for (const file of files) {
            try {
...
export function getPluginStats() {
    ...
    try {
        if (existsSync(WORKFLOWS_DIR)) {
            workflowPlugins = readdirSync(WORKFLOWS_DIR).filter((f) => f.endsWith('.json') || f.endsWith('.yaml') || f.endsWith('.yml')).length;
        }
    }
    catch { /* */ }
