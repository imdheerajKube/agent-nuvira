...
import { DEFAULT_FEDERATION_CONFIG, DEFAULT_FEDERATION_PORT, } from '../federation/protocol.js';
...
export class FederationCommand extends BaseCommand {
    ...
    async connectToServer(host, options) {
        const config = loadConfig();
        ...
        this.client.on('task-progress', (event) => {
    ...
    async runRemoteTask(goal, options) {
    ...
    async checkHealth() {
        ...
        const config = loadConfig();
        try {
            const response = await fetch(`http://${config.host}:${config.port}/federation/health`);
            const data = await response.json();
            // Unwrap envelope if present
            const healthData = (data?.type === 'response' ? data.payload : data);
            this.renderHealth({
