...
import { getSandboxConfig, setSandboxConfig, DEFAULT_RESOURCE_LIMITS, } from '../sandbox/types.js';
...
export class SandboxCommand extends BaseCommand {
    ...
    async manageConfig(options) {
        ...
        if (options.cpu !== undefined)
            limits.cpuLimit = options.cpu;
        if (options.disk)
            limits.diskLimit = options.disk;
        if (options.timeout !== undefined)
            limits.timeoutMs = options.timeout;
        if (options.network !== undefined)
            limits.networkAccess = true;
        const imageOption = options.image;
        if (imageOption) {
            // Try to find a matching built-in image
            const builtin = BUILTIN_SANDBOX_IMAGES.find((img) => img.image === imageOption || img.label.toLowerCase().includes(imageOption.toLowerCase()));
            if (builtin) {
        ...
        if (updates.limits)
            logger.info(`  Limits: memory=${updated.limits.memoryLimit}, cpu=${updated.limits.cpuLimit}, disk=${updated.limits.diskLimit}`);
        if (updates.image)
            logger.info(`  Image: ${updated.image.image}`);
    }
    listImages() {
        logger.highlight('═'.repeat(60));
    ...
    async runInSandbox(command, options) {
        ...
        try {
            ...
            const sandboxLimits = {};
            if (options.memory)
                sandboxLimits.memoryLimit = options.memory;
            if (options.cpu !== undefined)
                sandboxLimits.cpuLimit = options.cpu;
            if (options.timeout !== undefined)
                sandboxLimits.timeoutMs = options.timeout;
            if (options.network)
                sandboxLimits.networkAccess = true;
            // Use provided image or default from config
            const imageName = typeof options.image === 'string'
                ? options.image
                : getSandboxConfig().image.image;
            const containerId = await manager.createContainer(imageName, Object.keys(sandboxLimits).length > 0 ? sandboxLimits : undefined);
