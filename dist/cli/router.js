...
export function resolveProvider(configManager, providerOption) {
    ...
    if (rawType === 'auto') {
        const configuredFallback = ['groq', 'nim', 'gemini', 'openrouter', 'local'].find((p) => {
            try {
        ...
        logger.warn(`Provider 'auto' must be routed via Auto model routing — falling back to configured provider '${configuredFallback}'. ` +
