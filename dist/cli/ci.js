    ...
    async ciExecute(goal, options) {
        ...
        try {
            ...
            if (options.timeout && options.timeout > 0) {
                const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error(`Execution timed out after ${options.timeout}ms`)), options.timeout));
 ...
 *   - - [ ] ... (checklist items)
 */
export function parseReviewOutput(file, text) {
    const findings = [];
