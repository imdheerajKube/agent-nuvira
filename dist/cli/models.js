    ...
    create() {
        ...
        command
            ...
            try {
                ...
                if (g) {
                    if (g.denyProviders?.includes(provider))
                        governanceListed = true;
                    ...
                    if (!governanceListed && g.denyModels?.length) {
                        try {
                            ...
                            if (pin && g.denyModels.includes(pin))
                                governanceListed = true;
                        }
                        catch {
            ...
            if (governanceListed && !this.isJsonMode(opts, cmd)) {
                logger.warn(`   ⚠️ ${provider} is on an admin governance deny list (routing.governance.denyProviders/denyModels) — routing will STILL eliminate it even after this unblock. Edit routing.governance in .buffconfig.json to fully restore it.`);
            }
    ...
    async printVerboseStatus() {
        ...
        if (!tele.enabled) {
            ...
            for (const a of tele.actions) {
                const chips = [];
                if (a.verified > 0)
                    chips.push(`${a.verified} verified`);
                if (a.killed > 0)
                    chips.push(`${a.killed} killed`);
                if (a.transient > 0)
                    chips.push(`${a.transient} transient`);
