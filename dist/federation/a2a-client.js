...
import { A2A_TASK_TIMEOUT_MS, } from './a2a-types.js';
// ─── HTTP Helper ────────────────────────────────────────────────────────────
/**
 * Make an HTTP/HTTPS request and parse the JSON response.
 */
function makeRequest(url, method, body, timeoutMs = 10_000) {
...
export async function fetchAgentCard(baseUrl) {
    ...
    for (const path of ['/.well-known/agent-card', '/a2a/agent-card']) {
        try {
            const response = (await makeRequest(`${url}${path}`, 'GET'));
 ...
 */
export async function delegateTask(baseUrl, request, timeoutMs = A2A_TASK_TIMEOUT_MS) {
    const url = `${baseUrl.replace(/\/+$/, '')}/a2a/task`;
    const response = (await makeRequest(url, 'POST', request, timeoutMs));
    if (!response.taskId) {
 ...
 */
export async function pollTaskStatus(baseUrl, taskId, pollIntervalMs = 2_000, timeoutMs = A2A_TASK_TIMEOUT_MS) {
    ...
    while (Date.now() - startTime < timeoutMs) {
        try {
            ...
            if (response && response.error === true) {
                const msg = response.message || `Task ${taskId} not found`;
            ...
            if (status === 'completed' || status === 'failed') {
                ...
                return {
                    ...
                    summary: result?.summary || response.message || '',
 ...
 */
export async function delegateAndWait(baseUrl, request, options) {
    ...
    const result = await pollTaskStatus(baseUrl, response.taskId, options?.pollIntervalMs || 2_000, options?.timeoutMs || A2A_TASK_TIMEOUT_MS);
    if (options?.onProgress) {
 ...
 */
export async function checkA2AHealth(baseUrl) {
