...
import { FEDERATION_PROTOCOL_VERSION, DEFAULT_FEDERATION_CONFIG, } from './protocol.js';
const sessions = new Map();
const runningTasks = new Map();
const sseClients = new Map(); // taskId → SSE clients
let taskCounter = 0;
...
function setupSSEConnection(res, taskId) {
    res.writeHead(200, {
...
function broadcastTaskProgress(taskId, event) {
    const clients = sseClients.get(taskId);
    if (!clients)
        return;
...
// ─── Task Executor ──────────────────────────────────────────────────────────
async function executeDelegatedTask(task, taskId, abortController) {
...
async function handleTaskDelegation(req, res, _config) {
