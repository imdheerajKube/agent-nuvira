 ...
 */
export const BUILTIN_SANDBOX_IMAGES = [
    // ── Node.js ────────────────────────────────────────────────────────────
    {
        image: 'node:20-slim',
 ...
 */
export function resolveSandboxImage(runtime) {
    const normalized = runtime.toLowerCase().trim();
    // Exact match on label
    const byLabel = BUILTIN_SANDBOX_IMAGES.find((img) => img.label.toLowerCase().includes(normalized));
    if (byLabel)
        return byLabel;
    // Match on runtimes
    const byRuntime = BUILTIN_SANDBOX_IMAGES.find((img) => img.runtimes.some((r) => r.includes(normalized)));
    if (byRuntime)
        return byRuntime;
    // Match on image name
    const byImage = BUILTIN_SANDBOX_IMAGES.find((img) => img.image.toLowerCase().includes(normalized));
    if (byImage)
        return byImage;
    return undefined;
}
 ...
 */
export function detectProjectImage(projectDir) {
    // Check for common project files
    if (existsSync(join(projectDir, 'package.json')))
        return BUILTIN_SANDBOX_IMAGES[0]; // node:20-slim
    if (existsSync(join(projectDir, 'requirements.txt')) || existsSync(join(projectDir, 'setup.py')) || existsSync(join(projectDir, 'Pipfile')))
        return BUILTIN_SANDBOX_IMAGES[3]; // python:3.12-slim
    if (existsSync(join(projectDir, 'go.mod')))
        return BUILTIN_SANDBOX_IMAGES[5]; // golang:1.22
    if (existsSync(join(projectDir, 'Cargo.toml')))
        return BUILTIN_SANDBOX_IMAGES[6]; // rust:1.77-slim
