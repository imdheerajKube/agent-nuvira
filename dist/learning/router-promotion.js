...
function pendingKey(agentType, task) {
...
export class RouterPromotion {
     ...
     */
    noteParallelDecision(agentType, task, heuristic, bandit) {
     ...
     * trajectory file.
     */
    recordOutcome(agentType, task, outcome, outcomeData) {
     ...
     */
    evaluate(minDecisions = DEFAULT_MIN_PROMOTION_DECISIONS) {
        const decisions = this.getDecisions();
        const diverged = decisions.filter((d) => d.bandit.provider !== d.heuristic.provider ||
...
// ─── Singleton ──────────────────────────────────────────────────────────────
let promotionInstance = null;
/** Get or create the RouterPromotion singleton. */
export function getRouterPromotion() {
    if (!promotionInstance)
        promotionInstance = new RouterPromotion();
    return promotionInstance;
}
/** Reset the singleton (useful for testing). */
export function resetRouterPromotion() {
    promotionInstance = null;
}
//# sourceMappingURL=router-promotion.js.map