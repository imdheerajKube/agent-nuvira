 ...
 */
export function isRepairable(category) {
    switch (category) {
        case 'llm-error':
        case 'provider-error':
        case 'context-limit':
...
export class RepairBudget {
    ...
    maxTotal;
    constructor(maxPerTask = 3, mode = 'auto') {
 ...
 */
export function needsApproval(strategy, mode) {
    if (mode === 'off')
        return false;
    if (mode === 'auto')
        return false; // Auto mode: never requires approval
...
export class ErrorRepairEngine {
     ...
     */
    async repair(taskId, context, callLLM, originalError, executeFn) {
                ...
                switch (strategy) {
                    ...
                    case 'switch-model': {
                        ...
                        if (fallbackModel) {
                            ...
                            const fallbackLLM = async (prompt, opts) => {
                    ...
                    case 'adjust-temperature': {
                        ...
                        const lowTempLLM = async (prompt, opts) => {
 ...
 */
export function formatRepairSummary(taskId, category, finalResult, budget) {
...
//# sourceMappingURL=error-repair.js.map