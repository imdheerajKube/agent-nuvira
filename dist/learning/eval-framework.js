 ...
 */
export function scoreEvalMetrics(metrics) {
    ...
    return (EVAL_SCORE_WEIGHTS.testPass * testPassScore +
...
function readEvalData() {
    try {
        ...
        return JSON.parse(readFileSync(EVAL_PATH, 'utf-8'));
    }
    catch {
        return { runs: [], version: CURRENT_VERSION };
    }
}
function writeEvalData(data) {
    ensureDir();
...
export function scaffoldWorkspace(task) {
...
export function runHiddenTest(workspace, command, timeoutMs = 60_000) {
...
export function computeEditAccuracy(task, workspace) {
    const refs = task.referencePatterns ?? [];
    if (refs.length === 0)
        return 0; // no reference — no credit (conservative)
    let matched = 0;
    let total = 0;
 ...
 */
export async function runEvalTask(task, _provider, providerName, model, options = {}) {
    ...
    const perTestResults = task.hiddenTests.map((test) => {
        ...
        if (passed) {
            testsPassed += 1;
            if (firstGreenAt === null)
                firstGreenAt = Date.now() - taskStart;
        }
    ...
    try {
        ...
        metrics.costUsd = calculateCost(providerName, model, stats?.inputTokens ?? 0, stats?.outputTokens ?? 0);
    }
    catch {
...
export function computeEvalSummary(results) {
    if (results.length === 0) {
        return {
            totalTasks: 0,
            tasksPassed: 0,
...
export function formatEvalReport(run) {
    const s = run.summary;
    const lines = [];
...
export function formatEvalJSON(run) {
    return JSON.stringify(run, null, 2);
}
/** Format an eval run as Markdown. */
export function formatEvalMarkdown(run) {
    const s = run.summary;
    const lines = [
...
export function getLatestEvalRun(provider, model) {
    const data = readEvalData();
