 ...
 */
export function classifyFallbackError(err) {
 ...
 */
export function recordRegistrySuccess(providerType, model, action, latencyMs) {
    try {
...
export class ProviderFallback {
    ...
    pluginProviderCache = null;
    constructor(configManager, overrides) {
    ...
    getFallbackChain(primaryProvider) {
        ...
        const configured = this.fallbackConfig.providers.length > 0
    ...
    getProvider(providerType) {
        ...
        try {
            ...
            const resolved = this.configManager.getProviderConfig(providerType);
     ...
     */
    resolveConfiguredModel(providerType) {
        try {
     ...
     */
    async callWithFallback(primaryProvider, callFn, options) {
        ...
        for (let i = 0; i < Math.min(chain.length, maxAttempts); i++) {
            ...
            try {
                ...
                if (i >= Math.min(chain.length, maxAttempts) - 1) {
                    ...
                    throw new Error(`All providers exhausted after ${i + 1} attempts:\n` +
        ...
        throw new Error(`Provider fallback exhausted after ${attemptsMade.length} attempt(s)` +
 ...
 */
export function getProviderFallback(configManager, overrides) {
    if (!fallbackInstance) {
        if (!configManager) {
 ...
 */
export function resetProviderFallback() {
