...
export class PatternStore {
     ...
     */
    getByDomain(domainTags) {
        if (domainTags.length === 0)
            return this.getAll().slice(0, 3);
        return this.patterns
            .filter((p) => p.applicableDomains.some((d) => domainTags.some((tag) => d.toLowerCase().includes(tag.toLowerCase()))))
            .slice(0, 3);
    }
    /**
     * Format patterns as a prompt string for agent injection.
     */
    formatAsPrompt(domainTags) {
        const relevant = domainTags ? this.getByDomain(domainTags) : this.getAll().slice(0, 3);
        if (relevant.length === 0)
            return '';
        const parts = relevant.map((p, i) => `## Pattern ${i + 1}: ${p.title}\n` +
     ...
     */
    async extractFromTrajectories(trajectories, callLLM) {
        ...
        if (highScoring.length < 2)
            return 0; // Need at least 2 for pattern extraction
        ...
        for (const pattern of newPatterns) {
            ...
            const existing = this.patterns.findIndex((p) => p.title.toLowerCase() === pattern.title.toLowerCase());
            if (existing >= 0) {
    ...
    parsePatterns(response) {
        ...
        try {
            ...
            if (Array.isArray(parsed)) {
                return parsed.filter((p) => p.title && p.description && Array.isArray(p.applicableDomains));
            }
        }
        catch {
            // Fall through
