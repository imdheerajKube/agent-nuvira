 ...
 */
export function createReview(title, goal, changes, options) {
 ...
 */
export function addReviewComment(id, reviewer, status, comment) {
 ...
 */
export function mergeReview(id, workingDir) {
    const bundle = getReview(id);
    if (!bundle) {
        logger.error(`Review not found: ${id}`);
        return 0;
 ...
 * @returns The created ReviewBundle
 */
export function createReviewFromResult(goal, fileChanges, summary, options) {
    ...
    const changes = fileChanges.map((fc) => {
        ...
        if (fc.originalContent)
            change.originalContent = fc.originalContent;
        if (fc.newContent)
            change.newContent = fc.newContent;
