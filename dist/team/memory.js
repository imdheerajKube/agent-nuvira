...
function getTrajectoriesDir(cwd) {
    return join(getTeamDir(cwd), 'trajectories');
}
function getPatternsDir(cwd) {
 ...
 */
export async function initTeamMemory(repoUrl, cwd) {
    const dir = getTeamDir(cwd);
    ensureDir(dir);
    if (repoUrl) {
 ...
 */
export async function syncTeamMemory(cwd) {
    const result = { pulled: 0, pushed: 0, conflicts: [], errors: [] };
    if (!isGitRepo(cwd)) {
 ...
 */
export async function shareTrajectories(cwd) {
    ...
    const files = readdirSync(localTrajDir).filter((f) => f.endsWith('.json') || f.endsWith('.trajectory.json'));
    let shared = 0;
 ...
 */
export function getTeamMemoryStats(cwd) {
    const dir = getTeamDir(cwd);
