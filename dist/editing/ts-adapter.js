 ...
 */
export function findStructuralNodes(code, filePath) {
 ...
 */
export function findNodeByName(code, filePath, name) {
    const sourceFile = parseSourceFile(code, filePath);
    if (!sourceFile)
        return null;
    let found = null;
 ...
 */
export function findNodeAtPosition(code, filePath, line, column) {
    const sourceFile = parseSourceFile(code, filePath);
    if (!sourceFile)
        return null;
 ...
 */
export function nodeToRange(node, sourceFile) {
 ...
 */
export function getBodyRange(node, sourceFile) {
 ...
 */
function positionToOffset(code, sourceFile, line, column) {
 ...
 */
function visitNode(node, sourceFile, nodes, processed, language, currentDepth) {
 ...
 */
export function getNodeText(node, sourceFile, code) {
 ...
 */
export function replaceNodeText(code, node, sourceFile, newText) {
    const start = node.getStart(sourceFile);
    const end = node.getEnd();
 ...
 */
export function insertAt(code, sourceFile, line, column, text) {
    const pos = positionToOffset(code, sourceFile, line, column);
 ...
 */
export function validateTSSyntax(code, filePath) {
    if (!code || code.trim().length === 0)
        return true;
    try {
        const sourceFile = parseSourceFile(code, filePath);
