 ...
 */
import { positionToOffset, } from './types.js';
 ...
 */
export function applyEdit(code, edit) {
    switch (edit.type) {
        case 'replace-node':
...
export function applyEdits(code, edits) {
    ...
    for (const c of conflicts) {
        if (c.editIndex >= 0)
            conflictedIndices.add(c.editIndex);
        if (c.conflictingEditIndex >= 0)
            conflictedIndices.add(c.conflictingEditIndex);
    }
 ...
 */
function getEditRange(edit) {
    if (edit.targetNode?.range)
        return edit.targetNode.range;
    if (edit.textRange)
 ...
 */
function applyReplaceNode(code, edit) {
    if (!edit.targetNode)
        return applyRawReplace(code, edit);
    if (!edit.newCode)
        return null;
 ...
 */
function applyReplaceBody(code, edit) {
    if (!edit.targetNode?.bodyRange)
        return applyReplaceNode(code, edit);
    if (!edit.newCode)
        return null;
 ...
 */
function applyInsertBefore(code, edit) {
    if (!edit.targetNode)
        return null;
    if (!edit.newCode)
        return null;
 ...
 */
function applyInsertAfter(code, edit) {
    if (!edit.targetNode)
        return null;
    if (!edit.newCode)
        return null;
 ...
 */
function applyInsertChild(code, edit) {
    if (!edit.targetNode?.bodyRange)
        return null;
    if (!edit.newCode)
        return null;
 ...
 */
function applyDeleteNode(code, edit) {
    if (!edit.targetNode)
        return null;
    const start = positionToOffset(code, edit.targetNode.range.start);
    const end = positionToOffset(code, edit.targetNode.range.end);
 ...
 */
function applyAddImport(code, edit) {
    if (!edit.newCode)
        return null;
 ...
 */
function applyRawReplace(code, edit) {
    if (!edit.textRange)
        return null;
    if (!edit.newCode && edit.type !== 'delete-node')
        return null;
 ...
 */
export function formatEditSummary(filePath, edits) {
    const lines = [];
    lines.push(`📄 ${filePath}`);
