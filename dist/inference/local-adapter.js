...
`;
    ...
    async generateGGML(prompt, options) {
        ...
        if (!existsSync(model)) {
            throw new Error(`GGML model not found at: ${model}\n` +
        ...
        return new Promise((resolve, reject) => {
            ...
            let output = '';
            let errorOutput = '';
            llamaCpp.stdout?.on('data', (data) => {
                output += data.toString();
            });
            llamaCpp.stderr?.on('data', (data) => {
                errorOutput += data.toString();
            });
            llamaCpp.on('error', (err) => {
                if (err.code === 'ENOENT') {
                    reject(new Error('llama-cli not found. Install llama.cpp:\n' +
