...
import type { AgentProgress, AgentResult, FileChange, ExtensionMessage, WebviewMessage } from './types.js';

// ─── AgentPanel ─────────────────────────────────────────────────────────────

export class AgentPanel {
  public static readonly viewType = 'agent-nuvira.agentProgress';

  private panel: vscode.WebviewPanel | null = null;
  private disposables: vscode.Disposable[] = [];
  private currentResult: AgentResult | null = null;
  private onAcceptChanges?: (changes: FileChange[]) => void;
  private onRejectChanges?: (changes: FileChange[]) => void;
  ...
  showResult(result: AgentResult): void {
    ...
    if (result.success) {
      ...
      vscode.window.showInformationMessage(
        `✅ Agent completed: ${result.summary.slice(0, 100)}${numChanges > 0 ? ` (${numChanges} file change${numChanges !== 1 ? 's' : ''})` : ''}`,
      );
    } else {
