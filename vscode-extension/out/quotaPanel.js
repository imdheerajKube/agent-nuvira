...
exports.isQuotaWatchFile = isQuotaWatchFile;
 ...
 * armQuotaWatcher.
 */
function isQuotaWatchFile(filename) {
    const name = (0, node_path_1.basename)(String(filename || ''));
    return !name || name === 'quota-events.jsonl' || name === 'quota-ledger.json';
}
// ─── QuotaPanel ─────────────────────────────────────────────────────────────
class QuotaPanel {
    static viewType = 'agent-nuvira.quotaView';
    panel = null;
    disposables = [];
    loadStatus;
    ...
    constructor(options) {
        ...
        this.watchDir = options.watchDir || process.env.BUFF_MEMORY_DIR || (0, node_path_1.join)((0, node_os_1.homedir)(), '.buff', 'memory');
    ...
    createOrShow(extensionUri) {
        ...
        this.panel = vscode.window.createWebviewPanel(QuotaPanel.viewType, 'Agent-Nuvira Quotas', column, {
            enableScripts: true,
    ...
    armWatcher() {
        ...
        try {
            ...
            if (!(0, node_fs_1.existsSync)(this.watchDir)) {
                (0, node_fs_1.mkdirSync)(this.watchDir, { recursive: true });
            }
            this.watcher = (0, node_fs_1.watch)(this.watchDir, (_eventType, filename) => {
                if (!isQuotaWatchFile(filename))
                    return;
                if (this.watchTimer)
